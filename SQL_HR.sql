--                __________HR DATABASE__________
...
/* 18. Write a query to display the name (first_name, last_name) and salary for all employees whose salary is not in the range $10,000 through $15,000 
...
OR j.job_title = 'Shipping Clerk'
AND e.salary NOT IN(4500, 10000, 15000);


-- to get the orders from jan to mrch 2021
USE northwind;
SELECT * from orders;
...
AND MONTH(OrderDate) BETWEEN '01' AND '03';
...
SELECT street_address, 
	   SUBSTRING_INDEX(street_address, ' ', -1) AS 'Last_word_of_street_address' 
FROM locations;
...
SELECT job_title, SUBSTR(job_title,1, INSTR(job_title, ' ')) AS 'First_Word'
FROM jobs;
...
SELECT first_name, 
	   last_name,
	   LENGTH(first_name) 'Length_of_First_Name'
FROM employees
...
SELECT left(first_name, 8) 'First_8_char_of_first_name',
		salary,
...
--                __________NORTHWIND DATABASE__________
...
/* Most expensive to least expensive product list */
...
SELECT Discontinued, COUNT(*) AS 'Product Count'
FROM products
